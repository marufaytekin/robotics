...
class EulerRotation:
    ...
    def yaw(self, psi):
        ...
        return [[np.cos(psi), -np.sin(psi), 0],
    ...
    def rotate(self):
        ...
        for rotation in self._rotations:
            kind = rotation[0]
            angle = np.deg2rad(rotation[1])
            t = np.dot(self._rotation_map[kind](angle), t)
        return t



# Below you'll form a sequence of rotations. The rotation matrix `R` is
# the mapping of performing the rotations in sequential order. Multiplying
# a vector by `R` will perform the rotations on that vector.
...
# Go back and change the order of the rotations, you should get back a rotation matrix
# with different values.

# ### Same Rotations, Different Order
# 
# Now calculate three different rotations matrices. They should stem from
# the same set of Euler rotations, just in differing order.
...
# It's clear the three rotation matrices are different, but, in order to get
# an deeper understanding let's plot the impact of the rotations.
...
# ### Gimbal Lock
# To demonstrate gimbal lock try starting a series of rotations with a pitch
# of +/- 90 degrees, then see what happens when you try to yaw. Try the yaw
